...
## 사용자 결정 필요 (자율 범위 밖 — 릴리스 의미)
- **버전 번호 모순**: package.json=1.0.0, v1.0.0 태그 존재, 그러나 브랜치=0.23.0
  이고 CHANGELOG [Unreleased]가 0.23.0 작업으로 쌓이는 중. SemVer상 1.0.0 < 다음.
  해석 A) 0.23.0이 다음 릴리스 → v1.0.0 태그 시기상조(되돌릴지).
  해석 B) 0.23.0은 브랜치 식별자, 코드는 1.x 계속 → CHANGELOG를 1.1.0 등으로.
  단독 결정 안 함(릴리스 의미). 사용자 선택 후 버전 동기화.
